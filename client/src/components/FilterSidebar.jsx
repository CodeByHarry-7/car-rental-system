...
// ✅ FIX: Remove min_price and max_price from LOCAL_STATE_KEYS
// Now they can be set via FilterSidebar and useFilters hook
...
const CarListing = () => {
  ...
  // ✅ Updated: Now uses filters.min_price and filters.max_price from hook
  useEffect(() => {
    fetchCars();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(filters), currentDuration, filters.page]);
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      console.log("📦 Backend response:", {
        carsCount: res.data.cars?.length,
      ...
      if (carsData.length !== uniqueCars.length) {
        console.warn(
          `⚠️ Removed ${carsData.length - uniqueCars.length} duplicate car(s)`,
        );
      }
      ...
      if (currentPage > res.data.totalPages && res.data.totalPages > 0) {
        console.log(
          `🔄 Page ${currentPage} out of range, going to page ${res.data.totalPages}`,
        );
  ...
  const handlePriceChange = (e) => {
    ...
    setPriceValue(val);
    if (!selected || val === "") {
      // ✅ FIX: Clear price via hook
      setFilter("min_price", "");
      setFilter("max_price", "");
    } else {
      // ✅ FIX: Set price via hook
  ...
  const activeTags = [
    filters.location_id && {
      ...
      label: `📍 ${
        locations.find(
          (l) => String(l.id) === String(filters.location_id),
        )?.name || "Location"
      }`,
    },
          ...
          <FilterRow>
            ...
            <FilterChip>
              <FilterSelect
                ...
                onChange={(e) => {
                  setFilter("location_id", e.target.value);
                  setPage(1);
                }}
              >
            ...
            <FilterChip>
              <FilterSelect
                ...
                onChange={(e) => {
                  setFilter("transmission", e.target.value);
                  setPage(1);
                }}
              >
            ...
            <FilterChip>
              <FilterSelect
                ...
                onChange={(e) => {
                  setFilter("fuel_type", e.target.value);
                  setPage(1);
                }}
              >
            ...
            <FilterChip>
              <FilterSelect
                ...
                value={filters.seats || ""}
                onChange={(e) => {
                  setFilter("seats", e.target.value);
                  setPage(1);
                }}
              >
